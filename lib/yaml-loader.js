...
module.exports = (yamlText) => {
	const key = Array(100).fill().map(() => String.fromCodePoint(0x61 + Math.floor(Math.random() * 26))).join('');
	...
	return `module.exports = ${json.replace(new RegExp(`"#${key}#(.+?)#${key}#"`, 'g'), 'require("$1")')}`;
};
